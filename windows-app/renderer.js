...
function updateTimerLogic(forceUpdate = false) {
    ...
    // 遅延待機中は、現在届けられる固定時刻だけを新しい通知対象にする。
    if (isDelayedNotification && !isDelayedBySleepTime && nextNotificationTime && fixedTimes.length > 0) {
        const delayThreshold = 60000;
        const fixedTime = getNextFixedTime(new Date(now.getTime() - delayThreshold - 1));
        if (fixedTime && fixedTime > nextNotificationTime &&
            (!lastNotifiedTime || fixedTime.getTime() !== lastNotifiedTime.getTime()) &&
            now >= fixedTime.getTime() - notificationAdvanceSeconds * 1000 &&
            now <= fixedTime.getTime() + delayThreshold &&
            !(sleepTimeEnabled && sleepTimeStart && sleepTimeEnd && sleepTimeStart !== sleepTimeEnd &&
                applySleepTime(fixedTime).isInside)) {
            nextNotificationTime = fixedTime;
            lastScheduledSource = 'fixed';
        }
    }
