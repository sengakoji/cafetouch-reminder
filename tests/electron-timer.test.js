...
const source = fs.readFileSync(path.resolve(__dirname, '../windows-app/renderer.js'), 'utf8');
const fixedStart = source.indexOf('function getNextFixedTime');
...
const recalculateEnd = source.indexOf('// おやすみタイム判定ロジック', recalculateStart);
...
function createTimer({ now, oldTime = localTime(18, 12), fixedTimes = ['04:00', '16:00'], advance = 10,
    sleepDelayed = false, sleepEnabled = false, delayed = true, alreadyNotified = true } = {}) {
...
test('delayed Electron timer waits for a future fixed time, fires once, then completes and schedules normally', () => {
...
test('the Electron fixed-time recovery window includes 60 seconds but excludes later times', () => {
...
test('without fixed times or during sleep the Electron delayed state stays unchanged', () => {
...
test('ordinary Electron notification and retroactive delayed registration keep their existing flow', () => {
