...
> [!CAUTION]
> ### ⚠️ **PCブラウザ推奨です。Androidでの通知はまったく安定していません。iOSではテストできていません。**
...
**🌐 https://cafetouch-reminder.pages.dev/**
...
### 2.2 設定・カスタマイズ
*   **時間設定**: クールタイム（分）、カフェタッチにかかる(かける)時間（秒）、固定リセット時刻（複数登録可）。
*   **テーマ**: 
    *   カラー変更（プリセット + 自由選択）。
...
モバイルOS環境では、ブラウザのバックグラウンド動作制限（Dozeモード等）によりWeb Push通知が遅延・不達となる可能性が極めて高いため、環境に応じた利用方法を推奨・案内しています。

*   **Androidプラットフォーム**
    *   **専用APK版（野良アプリ）を強く推奨します**。ネイティブアプリとしてインストールすることでOSの設定（ポップアップ、バイブレーション等）と連携し、最も確実な通知を受け取ることができます。
    *   ブラウザやPWA版のままだと、OSの省電力機能により通知が正常に届かない可能性が高いです。
*   **iOS / iPadOSプラットフォーム**
    *   **「ホーム画面に追加」（PWA）が必須です**。ブラウザのままでは通知を受信できません。
    *   追加後、OS設定アプリの「通知」から当アプリを選び、通知バナーやサウンドを有効にする必要があります。
    *   クラウド（Cloudflare Workers）経由での配信となるため、多少の通知遅延が発生する場合があります。
